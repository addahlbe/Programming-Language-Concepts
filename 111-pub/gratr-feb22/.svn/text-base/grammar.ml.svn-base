...
type grammar = 
	 ...
	 * nullable_t (* which nonterminals are nullable *)
	) 
...
let print_grammar (os:string -> unit) (Grammar(name,start,ows,computed,ps,lexs,s)) : unit =
    os name; 
    os "\n\n";
    os "Start\n\n";
    os start;
...
let dump_grammar (os:string -> unit) (g:grammar) : unit =
  let Grammar(_,_,_,o,_,_,_) = g in
    ...
    (match o with
	   ...
	   print_stringset os syn_nts;
	   os "\n\nReading non-terminals (defined by lexical ->-productions):\n  ";
	   print_stringset os read_nts;
	   os "\n\nRecognizing non-terminals (defined by lexical =>-productions):\n  ";
	   print_stringset os recog_nts;
	   os "\n\nTerminal symbols:\n  ";
	   print_stringset os ter_syms;
	   os "\n\nNullable symbols:\n  ";
	   print_stringset os nullable;
	   os "\n\n";
    );
    os "\n\n";;
...
let to_ast (Gra2srs_syntax.Grammar(_,(_,name),_,(_,start),ows',(_,ops), (_,olx),(_,ovs),(_,ors))) : grammar =
  ...
  let nt_num = ref 0 in
  let new_nt(nt:string) = incr nt_num; !for_new_nt^nt^"_"^(string_of_int !nt_num) in
  ...
  let add_prod_n (name:string)(lhs:string)(rhs:string list) : unit =
	...
	| Some(pn) -> 
	    dbgc "add_prod_n: cached rule ";
	    dbgc lhs;
	    dbgc " -> ";
	    print_strings dbgc " " rhs;
	    dbgc "\n"
  in
  let add_prod (lhs:string)(rhs:string list) : unit =
    let pn = !for_new_rl^"_"^(string_of_int !pname_num) in
      incr pname_num; 
  ...
  and conv_atom (a:Gra2srs_syntax.atom) : string list =
    ...
    dbgc "(conv_atom ";
    Gra2srs_pp.pp_atom dbgc false a;
    dbgc "\n";
    let add_iter_rules (is_star:bool) (ss:string list) : string =
      let ntp = new_nt (if is_star then "_star" else "_plus") in
  ...
  let conv_lexs (lx:Gra2srs_syntax.lex list) : lexical list =
    ...
    List.rev !newls 
  in
  let syn_prods = conv_prods ps in
  ...
  in
  let opt_ws_str = match oopt_ws with None -> [] | Some(_,opt_ws) -> [opt_ws] in
  let rec process_side (is_lex:bool) ss = 
	...
	[] -> []
	  ...
	  let s = change_special s in
	    if is_quoted s then
	      (if is_lex then [] else opt_ws_str) @ (Util.explode_quoted s) @ (process_side is_lex ss')
	    else 
	      s::(process_side is_lex ss')
  in
  let (start,syn_prods) =
    (* if we are approximating the language only, then we omit the final _eos, just so
       we get exactly the expected automata. *)
    if !no_eos then
      (start,syn_prods)
    else
      ("_start",Prod("_Start","_start", ["_sos"; start;"_eos"])::syn_prods) in
  ...
  let ter_syms = 
    let ter = Trie.trie_new() in
      List.iter (fun (_,_,ss) -> List.iter (fun s -> if not (is_nt s) then stringset_add ter s) ss) all_sides;
      ter
  in
    
  let nullable =
    (* map non-terminals (lhs's) to their generated rhs's *)
    let index = trie_new() in
      List.iter (fun (_,s,ss) -> trie_cons index s ss) all_sides;

      dbgc "(Computing nullable nonterminals.\n";
      dbgc "(The index:\n";
      print_trie dbgc (fun l -> List.iter (fun s -> print_strings dbgc " " s; dbgc "; ") l) index;
      dbgc ")\n";
	...
	(* is a sequence of symbols currently known to be nullable *)
	let rec is_nullable s =
	  dbgc "(is_nullable "; 
	  dbgc s;
	  dbgc "\n";
	  let nullable_rhs rhs = 
	    dbgc "(nullable_rhs ";
	    print_strings dbgc " " rhs;
	    dbgc "\n";
	    let ret = and_list true (List.map is_nullable rhs) in
	      dbgc "nullable_rhs returning ";
	      dbgc (if ret then "true" else "false");
	      dbgc ")\n";
	      ret
	  in
	  let ret = 
	    if trie_contains mark s then
	      trie_contains nullable s
	    else
	      (stringset_add mark s;
	       let rhss = list_trie_lookup index s in
	       let r = or_list false (List.map nullable_rhs rhss) in
		 if r then
		   stringset_add nullable s;
		 r)
	  in
	    dbgc "is_nullable returning ";
	    dbgc (if ret then "true" else "false");
	    dbgc ")\n";
	    ret
	in
	let _ = is_nullable start in
	  nullable
  in
  ...
  in
    ...
    Grammar(name,start,ows,
	    (SyntacticNts(syn_nts), ReadingNts(read_nts), RecognizingNts(recog_nts), 
	     Terminals(ter_syms), Nullable(nullable)),

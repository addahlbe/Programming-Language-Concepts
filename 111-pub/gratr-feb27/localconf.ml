...
*)
let rec cps dbg (headmap:rule list trie) r1 r2 maxv (c:term -> term) (t:term) (eqlhs1:bool) (top_only:bool): cp list =
...
let cps_r (dbg:string -> unit) (headmap:rule list trie) (top_only:bool) (r : rule * rule) : cp list =
  ...
  let Rule((_,maxv),lhs,rhs) = r1' in
    dbgm "(cps_r computed maxv to be \n";
    dbgm (string_of_int maxv);
    dbgm " for these rules:\n  ";
    print_rule dbgm r1';
    dbgm "  ";
    print_rule dbgm r2;
    dbgm ")\n";
    
    cps dbg headmap r1' r2 maxv (fun x -> x) lhs true top_only;;

...
*)
let rec cps dbg (headmap:rule list trie) r1 r2 (c:term -> term) (t:term) (eqlhs1:bool) (top_only:bool): cp list =
  let dbgr = debugger dbg_rewriting in
  let Rule((oname1,maxv1),lhs1,rhs1) = r1 in
  let Rule((oname2,maxv2),lhs2,rhs2) = r2 in
  ...
  let rec cps_args (f:string) (pre_args:term list) (rest_args : term list) : cp list =
	...
	[] -> []
      | arg::rest_args' ->
	  (cps dbg headmap r1 r2 (fun x -> c (App(f,pre_args @ x::rest_args'))) arg false false)
	  @ (cps_args f (pre_args @ [arg]) rest_args')
  in
    match t with
	Var(x) -> [] (* we do not use unification with a variable in lhs1 to generate a cp *)
      | Normal(t') -> cps dbg headmap r1 r2 c t' eqlhs1 top_only
      | App(f,args) ->
	  let rest() = if top_only then [] else cps_args f [] args in
	    if (compare_rules r1 r2) = 0 && eqlhs1 then
	      ...
	      (dbg "(Trying to unify the subterm\n  ";
	       print_term dbg t;
	       dbg "\nof the lhs of rule\n  ";
	       print_rule dbg r1;
...
*)
let cps_r (dbg:string -> unit) (headmap:rule list trie) (top_only:bool) (r : rule * rule) : cp list =
  let (r1,r2) = r in
  let Rule((_,maxv2),_,_) = r2 in
  let r1' = rename (maxv2+1) r1 in
  let Rule(_,lhs,rhs) = r1' in
    cps dbg headmap r1' r2 (fun x -> x) lhs true top_only;;
...
let locally_confluent (Trs(rs)) : (cp list) =
  ...
  let dbgm = debugger dbg_local_conf_micro in
    dbg "(Beginning check for local confluence.\n\n";
	...
	dbg "Check for local confluence ends.)\n";
	ret
;;
